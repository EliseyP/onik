...
- Переводит текст с различными <strong>Orthodox \[UCS\]</strong> шрифтами в <strong>Ponomar Unicode</strong>.
- Переводит текст из <strong>Ponomar Unicode</strong> в <strong>\[UCS\]</strong> (с возможностью отделить надстрочники). 
...
- Меняет _(циклично)_ буквы <strong>[и,ї,ы] [о,ѡ] [е,є]</strong> для мн. и ед. числа: <strong>-щимъ -щымъ; последняя е <-> є; последняя о <-> ѡ</strong> 
...
Для тестов или скриптов м.б. использованы py-скрипты c функциями обработки Ponomar-текста:   
``$ onik_run.py 'Фома'``  
``Ѳѡма̀``     
и текстовый фильтр:   
...
#### Конвертер для командной строки без использования LibreOffice 
В каталоге **utils** находится python-скрипт:  
``convert_xml.py``  
...
#### При обновлении:
- В версии 1.1.78 поменялась функция, привязанная к hotkey CTRL+ALT+P.  
<strong>Решение:</strong> через меню Оник_ЦСЯ выбрать "Удалить все горячие клавиши". Затем там же установить их.  
