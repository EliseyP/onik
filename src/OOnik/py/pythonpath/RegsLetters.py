...
regs_letters_in_word = (
    ...
    [r'\b(свѧщенноиспов)ед(?=ни)', r'\1ѣд', 'i'],
    ...
    [r'((?:є|архіе)п)и(?=скоп)', r'\1ї', 'i'],
    # [r'(архіеп)и(?=скоп)', r'\1ї', 'i'],
    ...
    [r'\b(м)и(?=ро\b|ро(нос|точ|полож|дохнов|ꙋхан|помаз))', r'\1ѵ', 'i'],
    ...
    # -ного -> -наго # кроме: ѻ҆дногѡ̀  ѻ҆́ного(?) много
    [r'((?<![ѻд])н)о(?=г[оѡ]\b)', r'\1а'],
    [r'(\b(пре)?мн)а(?=г[оѡ]\b)', r'\1о'],

    # -кого -> -каго; кроме: кого никого
    [r'\B((?<!ни)к)о(?=г[оѡ]\b)', r'\1а'],
    ...
    [r'''
        \b
        (
          ба|бас|бол[еѣ]з|бра|верх|(не)?вечер|господ|голен|д|днеш|єле|зад|зд[еѣ]ш|и|ѵме|іри|каз|ки|кѵри|ко|лім|милосты|нын[еѣ]ш|перст|пламен|прежд?|п[еѣ]с|сви|ран|саже|ста|степе|тверды|хана
        )
        нѣ(?=й\b)
        ''', r'\1не'],

    # В словах крестообразный
    [r'оѡ', r'оѻ'],
